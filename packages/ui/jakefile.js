...
async function runEsbuild(options = {}) {
    ...
    const result = await esbuild
        .build({
            ...
            define: {
                "process.env.NODE_ENV": options.dev
                    ? "'production'"
                    : "'development'",
...
task(
    "build-npm",
    ["clean", "build-ts", "esbuild-cjs"],
    sh`api-extractor run --local --verbose`,
);

task(
    "upload",
    ["clean", "css", "esbuild-esm"],
    sh`

        aws s3 cp --recursive cdn-dist "s3://findkitshared-jscdnfilebucket1f0d03b0-1k0pb9d1j2lk4/novus/$(git rev-parse HEAD)"
    `,
);

task("css", sh`pnpm run css`);
